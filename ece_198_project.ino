...
void loop() {
  ...
  if (SPO2Valid && (SPO2 != -999) && heartValid && (heartRate != -999)) {
    ...
    Serial.println("heart rate error");
    lcd.clear();
    lcd.setCursor(0, 0);
  ...
  if (magnitude - 9.81 > thresholdAccel) {                                            //disclude acceleration from gravity
  ...
  if (nightReadingsPulseOximeter[pulseIndex - 1].heartRate < thresholdHeartRate[2]) {
  ...
  ssiPulse = (ssiPulse / 1.0) * (0 - 35) + 35;

  ssiAccel = (abs(nightReadingsAccelerometer[accelIndex - 1].magnitude - thresholdAccel) / thresholdAccel) * 100;
  ssiAccel = (ssiAccel / 1.0) * (0 - 30) + 30;


  ssiSound = (abs(nightReadingsSound[soundIndex - 1].sound - thresholdSound) / thresholdSound) * 100;
  ssiSound = (ssiSound / 1.0) * (0 - 35) + 35;

  SSI = ssiPulse + ssiAccel + ssiSound;


  // Threshold Comparisons
  if (nightReadingsPulseOximeter[pulseIndex - 1].heartRate  == 0 ||
      nightReadingsAccelerometer[accelIndex - 1].magnitude == 0 ||
      nightReadingsSound[soundIndex - 1].sound == 0) {
    errorInstability = 1; 
    severeInstability = 0;
    highInstability = 0;
    mildInstability = 0;
  ...
  else if (nightReadingsPulseOximeter[pulseIndex - 1].heartRate < 0.15 * thresholdHeartRate[0] || 
  nightReadingsPulseOximeter[pulseIndex - 1].heartRate > 0.15 * thresholdHeartRate[1] || 
  nightReadingsAccelerometer[accelIndex - 1].magnitude > 0.15 * thresholdAccel || 
  nightReadingsSound[soundIndex - 1].sound > 0.15 * thresholdSound) {
    // HIGH INSTABILITY!
    errorInstability = 0; 
    highInstability = 1;
    severeInstability = 0;
  ...
  else if (nightReadingsPulseOximeter[pulseIndex - 1].heartRate < 0.1 * thresholdHeartRate[0] || 
  nightReadingsPulseOximeter[pulseIndex - 1].heartRate > 0.1 * thresholdHeartRate[1] || 
  nightReadingsAccelerometer[accelIndex - 1].magnitude > 0.1 * thresholdAccel || 
